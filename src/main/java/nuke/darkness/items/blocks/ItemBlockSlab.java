package nuke.darkness.items.blocks;
...
public class ItemBlockSlab extends ItemBlock implements IModeledItem {
	        ...
	        float hitX, float hitY, float hitZ )
	{
		...
		if (stack.getCount() == 0) {
			...
			if (ibs.getBlock() == getBlock()) {
				...
				if (facing == EnumFacing.UP && ebh == BlockSlab.EnumBlockHalf.BOTTOM
				        || facing == EnumFacing.DOWN && ebh == BlockSlab.EnumBlockHalf.TOP)
				{
					IBlockState ibs1 = this.doubleSlab.getDefaultState();

					if (world.checkNoEntityCollision(this.doubleSlab.getBoundingBox(ibs1, world, pos))
					        && world.setBlockState(pos, ibs1, 3))
					{
		...
		return this.tryPlace(stack, world, pos.offset(facing))
		        || (super.onItemUse(player, world, pos, hand, facing, hitX, hitY, hitZ) == EnumActionResult.SUCCESS
		                ? true
		                : false) ? EnumActionResult.SUCCESS : EnumActionResult.FAIL;
	}
	...
	private boolean tryPlace( ItemStack stack, World world, BlockPos pos ) {
		...
		if (ibs.getBlock() == getBlock()) {
			IBlockState ibs1 = this.doubleSlab.getDefaultState();
			if (world.checkNoEntityCollision(this.doubleSlab.getBoundingBox(ibs, world, pos))
			        && world.setBlockState(pos, ibs1, 3))
			{
				world.playSound(pos.getX() + 0.5f, pos.getY() + 0.5f, pos.getZ() + 0.5f,
				        this.doubleSlab.getSoundType().getPlaceSound(), SoundCategory.BLOCKS,
