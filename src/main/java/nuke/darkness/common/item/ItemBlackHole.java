...
public class ItemBlackHole extends ItemBasic implements IInventoryShadeBlackhole, IHeldShadeBlackhole, IShadeItem {
	...
	public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
		...
		if (ShadeInventoryUtil.getShadeTotal(player) >= 100.0 || player.capabilities.isCreativeMode) {
			...
			if (DKeybinds.CHARGE_BLACKHOLE.isKeyDown() && !(ShadeInventoryUtil.getShadeTotal(player) == 1000.0)) {
	...
	public void addInformation(ItemStack stack, World player, List<String> tooltip, ITooltipFlag flag) {
		tooltip.add(TextFormatting.DARK_PURPLE + TextUtil.localizeEffect("tooltip.darkness:blackhole"));
		if (stack.hasTagCompound() && flag.isAdvanced()) tooltip.add("" + getShade(stack) + "/" + getShadeCapacity(stack));
	}
	...
	public double getShade(ItemStack stack) {
		...
		return stack.getTagCompound().getDouble(Darkness.resourcePrefix("shade"));
	}

	@Override
	public double getShadeCapacity(ItemStack stack) {
		if (!stack.hasTagCompound()) initNBT(stack);
		return stack.getTagCompound().getDouble(Darkness.resourcePrefixWithCapacity("shade"));
	}

	@Override
	public void setShade(ItemStack stack, double val) {
		if (!stack.hasTagCompound()) initNBT(stack);
		stack.getTagCompound().setDouble(Darkness.resourcePrefix("shade"), val);
	}

	@Override
	public void setShadeCapacity(ItemStack stack, double val) {
		if (!stack.hasTagCompound()) initNBT(stack);
		stack.getTagCompound().setDouble(Darkness.resourcePrefixWithCapacity("shade"), val);
	}
	...
	public double addAmount(ItemStack stack, double val, boolean add) {
		...
		double shade = stack.getTagCompound().getDouble(Darkness.resourcePrefix("shade"));
		double capacity = stack.getTagCompound().getDouble(Darkness.resourcePrefixWithCapacity("shade"));
